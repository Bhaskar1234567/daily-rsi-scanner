...
def get_symbols(limit=200):
    ...
    # CoinDCX returns futures instruments as pair strings such as B-BTC_USDT.
    ...
    # Current futures prices endpoint supplies ls=last price and v=24h quantity volume.
    pr = SESSION.get(
...
def previous_day_levels(daily):
    ...
    # Red = LONG level: (Previous Day High + Previous Day Close) / 2
    long_level = (row["high"] + row["close"]) / 2

    # Green = SHORT level: (Previous Day Open + Previous Day Low) / 2
    short_level = (row["open"] + row["low"]) / 2

    return long_level, short_level, row["time"]


def transition_signal(df, long_level, short_level):
    """
    3M Previous-Day OHLC clean crossing logic.

    LONG level  = (Previous Day High + Previous Day Close) / 2
    SHORT level = (Previous Day Open + Previous Day Low) / 2

    LONG only when price comes from BELOW the LONG level and a
    completed 3M candle CLOSES above it.

    SHORT only when price comes from ABOVE the SHORT level and a
    completed 3M candle CLOSES below it.

    Wicks/touches and crossings of the wrong level do not create signals.
    """
    found = None
    previous_close = None
    ...
    for _, row in df.iterrows():
        ...
        if previous_close is not None:
            # LONG: previous close below red/long level -> current close above it
            if previous_close < long_level and close > long_level:
                found = ("LONG", row)

            # SHORT: previous close above green/short level -> current close below it
            elif previous_close > short_level and close < short_level:
                found = ("SHORT", row)
...
def scan_rsi(pair, timeframe, period, ob, near_ob, os, near_os, volume):
    ...
    previous_vol = float(df["volume"].iloc[-2]) if len(df) >= 2 else 0.0
    vol_pct = ((current_vol - previous_vol) / previous_vol * 100.0) if previous_vol > 0 else 0.0
    vol_dir = "UP" if current_vol > previous_vol else "DOWN" if current_vol < previous_vol else "FLAT"
    ...
    return {
        ...
        "Volume Trend": vol_dir,
        "Volume Trend %": vol_pct
    }
...
def scan_ohlc(pair, volume):
    ...
    current_vol = float(m3["volume"].iloc[-1])
    previous_vol = float(m3["volume"].iloc[-2]) if len(m3) >= 2 else 0.0
    vol_pct = ((current_vol - previous_vol) / previous_vol * 100.0) if previous_vol > 0 else 0.0
    vol_dir = "UP" if current_vol > previous_vol else "DOWN" if current_vol < previous_vol else "FLAT"
    ...
    return {
        ...
        "LONG Level": float(up), "SHORT Level": float(down),
...
if not df.empty:
    ...
    def signal_rows(part, title, key_prefix, ohlc=False):
        ...
        if part.empty:
            ...
            return
        if not ohlc:
            h1, h2, h3, h4, h5, h6 = st.columns([1.45, 1.15, 1.35, 1.55, 1.25, 0.9])
            h1.caption("Coin")
            h2.caption("RSI")
            h3.caption("Price")
            h4.caption("Volume")
            h5.caption("Vol Trend")
            h6.caption("Actions")

        for _, row in part.iterrows():
            coin, pair = row["Coin"], row["Pair"]
            c1, c2, c3, c4, c5, c6 = st.columns([1.45, 1.15, 1.35, 1.55, 1.25, 0.9])
            c1.write(f"**{coin}**")

            if ohlc:
                c2.write(f"LONG **{row['LONG Level']:,.6g}**")
                c3.write(f"SHORT **{row['SHORT Level']:,.6g}**")
            ...
            vol_dir = row.get("Volume Trend", "FLAT")
            vol_pct = float(row.get("Volume Trend %", 0.0))
            if vol_dir == "UP":
                c5.markdown(
                    f'<span style="color:#168a16;font-weight:700;font-size:16px">▲ +{abs(vol_pct):.0f}%</span>',
                    unsafe_allow_html=True,
                )
            elif vol_dir == "DOWN":
                c5.markdown(
                    f'<span style="color:#d71920;font-weight:700;font-size:16px">▼ -{abs(vol_pct):.0f}%</span>',
                    unsafe_allow_html=True,
                )
            else:
                c5.markdown(
            ...
            if c6.button("📊 Chart", key=f"show_{key_prefix}_{coin}"):
